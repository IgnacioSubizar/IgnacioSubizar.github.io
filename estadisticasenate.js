...
if(miembros.party=="I"){
 ...
 if(miembros.party=="D" || miembros.party=="R" || miembros.party=="I"){
    app.statistics.TotSenate.push(miembros)}
})

console.log("Number of Republicans: " + app.statistics.NumberofRepublicans.length);
console.log("Senate Total: " + app.statistics.TotSenate.length);  
...
app.statistics.totalSen= app.statistics.TotSenate.length;
}
...
s.forEach(miembros=>{
    ...
    if(miembros.party=="I"){
        sumPctI += miembros.votes_with_party_pct
           ...
           app.statistics.AverageVoteswithpartyforIndependents=(sumPctI/app.statistics.totalInd).toFixed([2])



var totpctparty = (((sumPctI/app.statistics.NumberofIndependents.length)+(sumPctR/app.statistics.NumberofRepublicans.length)+
(sumPctD/app.statistics.NumberofDemocrats.length))/3).toFixed([2])
app.statistics.totAvrgVotes=totpctparty

console.log("Average Votes Total: " + app.statistics.totAvrgVotes)
console.log("Average Votes with party for Democrats: " + app.statistics.AverageVoteswithpartyforDemocrats)
console.log("% Party Total: " + totpctparty)
